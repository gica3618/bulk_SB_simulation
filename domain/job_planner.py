#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  5 15:41:43 2026

@author: gianni
"""

from dsa_ha_policy import DSAHourAnglePolicy
from domain.simulation_job import SimulationJob


class JobPlanner:

    def __init__(self, sb, default_step, fine_step):
        self.sb = sb
        self.default_step = default_step
        self.fine_step = fine_step
        self.DSA_HA = DSAHourAnglePolicy.compute(sb)

    def compute_HAs(self, step):
        if step.hour <= 0:
            raise ValueError(f"invalid HA step: {step.hour} H")
        #min_HA considered by DSA might be affected by PolCal, but max_HA is
        #always an integer, so I start with max_HA
        if not self.DSA_HA["max"].hour.is_integer():
            raise RuntimeError("expected max DSA HA to be an integer, but it is"
                               +f"{self.DSA_HA['max'.hour]}h")
        HAs = []
        HA = self.DSA_HA["max"]
        while HA >= self.DSA_HA["min"]:
            HAs.append(HA)
            HA -= step
        return HAs.sort()

    def jobs(self, xml_filepath, array_config, date, step):
        jobs = []
        for ha in self.compute_HAs(step):
            job = SimulationJob(xml_filepath=xml_filepath,
                                array_config=array_config,
                                HA=ha,
                                date=date)
            jobs.append(job)
        return jobs

    def default_job_grid(self,xml_filepath,array_config,date):
        return self.jobs(xml_filepath=xml_filepath, array_config=array_config,
                         date=date, step=self.default_step)

    def fine_job_grid(self,xml_filepath,array_config,date):
        return self.jobs(xml_filepath=xml_filepath, array_config=array_config,
                         date=date, step=self.fine_step)