...
def test_calibrator():
    ...
    calibrator.Calibrator(name="test", source_name="query", cal_type="Bandpass",
                          is_hardcoded=False,coordinates=None)
    calibrator.Calibrator(name="test", source_name="J123", cal_type="DGC",
                          is_hardcoded=True,coordinates=coord)
    with pytest.raises(ValueError):
        calibrator.Calibrator(name="test", source_name="query", cal_type="Bandpass123",
                              is_hardcoded=False,coordinates=None)
    with pytest.raises(ValueError):
        calibrator.Calibrator(name="test", source_name="query", cal_type="Bandpass",
                              is_hardcoded=False,coordinates=coord)
    with pytest.raises(ValueError):
        calibrator.Calibrator(name="test", source_name="Jirgendwas", cal_type="Bandpass",
                              is_hardcoded=True,coordinates=None)


class TestGetCalibratorCandidates:

    test_output_folder = "tests/getCalibratorCandidates_outputs"
    mock_xml_filepath = "something.xml"
    general_get_cal_candidates = calibrator.GetCalibratorCandidates(
    ...
    def get_calibrator_candidates(self,filename):
        filepath = os.path.join(self.test_output_folder,filename)
    ...
    def test_calibrator_candidates_results(self):
        ...
        expected_calibrator = calibrator.CalibratorCandidate(
